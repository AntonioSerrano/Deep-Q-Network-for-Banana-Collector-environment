...
if dueling:
    class QNetwork(nn.Module):
        ...
        def __init__(self, state_size, action_size, seed, fc1_units=64, fc2_units=64):
            ...
            # Same CNN architecture as in Mnih et al. (2015) in Nature's paper: https://web.stanford.edu/class/psych209/Readings/MnihEtAlHassibis15NatureControlDeepRL.pdf
            # i.e. 3 conv layers (with no pooling or dropout layers) with 
            # 2 fully-connected layers at the end.
            # input size = torch.Size([1, 4, 84, 84]) # 64 images per batch, 4 stacked images, 84 pixels wide, 84 pixels high
            ...
            self.fc1_adv = nn.Linear(64*9*9, 512)
            self.fc1_val = nn.Linear(64*9*9, 512)
        ...
        def forward(self, state):
            ...
            # state shape is torch.Size([1, 4, 84, 84])
            y = F.relu(self.conv1(state))
            y = F.relu(self.conv2(y))
            y = y.view(y.size(0), -1) # flatten previous output for the fully-connected layer

            adv = F.relu(self.fc1_adv(y))
            val = F.relu(self.fc1_val(y))
